...
const NoteToolbar = ({
    ...
    if (note.has_password) {
      return (
        <ToolbarItem
          icon={<Lock />}
          title="Locked"
          onClick={onPasswordOptionsPress}
        />
      )
    }
  ...
  const renderSpellcheckItem = () => {
    if (spellcheck) {
      return (
        <ToolbarItem
          icon={<TextFormat />}
          tooltip="Disable Spellcheck"
          onClick={onSpellcheckTogglePress}
        />
      )
    }
        ...
        <form onSubmit={handleChangeNotePassword}>
          ...
          <DialogContent>
            ...
            <PasswordInput
              ...
              onChange={event => {
                setNewNotePassword(event.target.value)
              }}
            />
          </DialogContent>
          <DialogActions>
    ...
    apiClient(
      ...
      .catch(error => {
        ...
        if (
          ...
          toast.error(capitalizeFirstLetter(error.response.data.path[0]), {
            autoClose: false
          })
        }
      })
  ...
  const renderChangeFontSizeModal = () => {
    return (
      <Dialog
        open={changeFontSizeModal}
        onClose={() => {
          setChangeFontSizeModal(false)
        }}
        aria-labelledby="form-dialog-title">
        ...
        <AppBar position="static">
          <Toolbar variant="dense" className={classes.toolbar}>
            ...
            <ToolbarItem
              icon={<Code />}
              title="Code"
              onClick={onCodeViewPress}
            />
            <ToolbarItem
              icon={<Pageview />}
              title="Markdown"
              onClick={onMarkdownViewPress}
            />
            <Divider
              className={classes.divider}
              orientation="vertical"
