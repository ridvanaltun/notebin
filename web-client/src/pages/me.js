...
const Me = () => {
  ...
  const onNoteTrackingBackupPress = async (event, row) => {
    try {
      ...
      const b = {
        ...res.data,
        updated_at: timeAgo(res.data.updated_at),
        created_at: timeAgo(res.data.created_at)
      }
  ...
  const onBackupDeletePress = oldData => {
    return new Promise((resolve, reject) => {
      apiClient({
        ...
        .catch(error => {
          toast.error('Backup did not delete. An error occured!', {
            autoClose: false
          })
          reject(error)
        })
  ...
  const renderBackupIdColumn = rowData => {
    return <Typography>{rowData.unique_id.substring(0, 8) + ' ...'}</Typography>
  }
  ...
  const renderTrackingPathColumn = rowData => {
    return (
      <Link href={`/${rowData.path}`}>
        <a target="_blank">
          {rowData.path.length > 8
            ? `${rowData.path.substring(0, 8) + ' ...'}`
            : rowData.path}
        </a>
      </Link>
    )
  ...
  const renderBackupOrginalPathColumn = rowData => {
    return (
      <Link href={`/${rowData.original_path}`}>
        <a target="_blank">
          {rowData.original_path.length > 8
            ? `${rowData.original_path.substring(0, 8) + ' ...'}`
            : rowData.original_path}
        </a>
      </Link>
    )
  }
  ...
  const renderBackupDate = rowData => {
    return <Typography>{timeAgo(rowData.updated_at)}</Typography>
  }
        ...
        <Box m={5}>
          ...
          <form onSubmit={handleProfileUpdate}>
            <TextField
              ...
              helperText={
                profileUpdateClicked && error && error.first_name
                  ? error.first_name
                  : ''
              }
            />
            <TextField
              ...
              helperText={
                profileUpdateClicked && error && error.last_name
                  ? error.last_name
                  : ''
              }
            />
        ...
        <Box m={5}>
          ...
          <form onSubmit={handleEmailUpdate}>
            <TextField
              ...
              helperText={
                emailChangeClicked && error && error.email ? error.email : ''
              }
            />
            <Button
              type="submit"
    ...
    return (
      <Box className={classes.changePasswordContainer}>
        <Accordion
          ...
          <AccordionDetails>
            <Typography>{renderEditProfile()}</Typography>
          </AccordionDetails>
        </Accordion>
        <Accordion
          ...
          <AccordionDetails>
            <Typography>{renderChangePassword()}</Typography>
          </AccordionDetails>
        </Accordion>
        <Accordion
          ...
          <AccordionDetails>
            <Typography>{renderDangerActions()}</Typography>
          </AccordionDetails>
        </Accordion>
      </Box>
    )
        ...
        columns={[
          {
            field: 'path',
            title: 'Path',
            render: renderTrackingPathColumn,
            searchable: true
          },
        ...
        columns={[
          {
            field: 'unique_id',
            title: 'Unique ID',
            render: renderBackupIdColumn
          },
          {
            field: 'original_path',
            title: 'Original URL',
            render: renderBackupOrginalPathColumn
          },
      ...
      <Box>
        <h1>
          <b>{`${capitalizeFirstLetter(
            user.first_name
          )} ${capitalizeFirstLetter(user.last_name)} `}</b>
          {`(@${user.username})`}
        </h1>
        <Divider light />
        <Box m={1} mt={3}>
  ...
  return (
    <Box className={isPhoneOrTablet ? classes.smallRoot : classes.root}>
      ...
      <Paper
        variant="outlined"
        className={
          isPhoneOrTablet ? classes.smallTabsCard : classes.rightSection
        }>
        {renderTabs()}
      </Paper>
    </Box>
