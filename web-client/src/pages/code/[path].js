...
const CodePreview = ({path, noteInfo, hasPassword, defaultLanguage}) => {
  ...
  useEffect(() => {
    router.push(`/code/${path}?language=${language}`, undefined, {
      shallow: true
    })
  }, [])
  ...
  const handleLanguageChange = newLanguage => {
    ...
    router.push(`/code/${path}?language=${newLanguage}`, undefined, {
      shallow: true
    })
  }
...
CodePreview.getInitialProps = async ctx => {
  ...
  try {
    ...
    if (noteHasPassword)
      return {hasPassword: true, noteInfo: {text: ''}, path, defaultLanguage}
